...
public class Pawn extends Piece {
	...
	public Pawn(String pawnID, boolean team, Board b, int col, int row) {
		this.setRow(row);
		this.setCol(col);
		String loc = Character.toString((char) ((char) row + 'A'));
		loc = loc + Integer.toString(col + 1);
		// System.out.println("Pawn internal loc set as " + loc + ", line 17 in Pawn");
		// //- tested and works, 12/1/2019, certified not a problem
	...
	public ArrayList<BoardButton> getMoves(Piece piece, BoardButton[][] board) {
		...
		// FIXME - This is probably not right, this references line 56 of BoardButton.
		// ONLY pawn was having an off by one issue. 12/1/2019
		...
		if (team) { // White pawn, goes up
			...
			if (p.firstMove()) {
				...
				if (front != null && !front2.isFull() && !front.isFull()) {
					// System.out.println("Pawn's first move! Should add " +
					// front2.getAbbreviation() + " to list!");
	...
	public boolean canPromote() {
		if (this.rank == 5) {
			// BoardButton b;
			if (this.isWhite) {
				if (!bb[this.col][++this.row].isFull())
					return true;
				else
					return false;
			} else {
	...
	/**
	 * @author Henry Rheault Helper Method for do-while in promote() method, returns
	 *         false if invalid or pulls char out of the s.nextLine returns false if
	 *         pawn or king entered, returns null as default
	 */
	...
	/**
	 * @author Henry Rheault Handles promotions, if rank == 6 (after it moves to the
	 *         end of the board) will handle dialog of what piece to construct in
	 *         it's place (Realistically only Queen, or Knight in the case of a
	 *         super big brain gamer) Returns piece created to be added to the board
	 */
