...
public class Move {
	...
	public Move(Piece p, BoardButton button, boolean thisIsAMoveGeneratorOnlyWillNotPushAMove) throws Exception { // Overloaded
																													// //
																													// boardbutton
		...
		try {
			...
			// System.out.println("Line 92 in Move, " + p.getAbbrev() + " moved from " +
			// loc[0] + loc[1] + " to " + abbrev + ".");
		...
		try {
			...
			if (this.piece.getAbbrev() == 'p' || this.piece.getAbbrev() == 'P') {
				if (this.piece.firstMove()) {
					...
					if (Math.abs(this.n3w.getColumn() - this.old.getColumn()) == 2)
						this.getPiece().incRank();
				}
	...
	public Move(Pawn p) {
		...
		// System.out.println("Calling remove on boardbutton" + i + " " + j);
		if (p.isWhite)
			this.n3w = GameBoard[p.getCol()][7];
		if (!p.isWhite)
			this.n3w = GameBoard[p.getCol()][0];
	...
	public Move(Pawn p, boolean side) {
		...
		if (p.isWhite) {
			if (side)
				this.n3w = GameBoard[p.getCol() + 1][7];
			else
				this.n3w = GameBoard[p.getCol() - 1][7];
		}
		if (!p.isWhite) {
			if (side)
				this.n3w = GameBoard[p.getCol() + 1][0];
			else
				this.n3w = GameBoard[p.getCol() - 1][0];
		}
	...
	/**
	 * @author Henry Rheault Generates and returns a move based on a input string.
	 *         Passed in
	 * @throws Exception
	 */
